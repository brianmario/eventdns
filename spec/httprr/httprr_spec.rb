...
describe HTTPRR do
  ...
  it "correctly validates NAME values" do
    HTTPRR.new(@rr.merge({'name'=>''})).valid?.should == true
    HTTPRR.new(@rr.merge({'name'=>'X'*255})).valid?.should == true
    HTTPRR.new(@rr.merge({'name'=>'X'*256})).valid?.should == false
  end

  it "correctly validates TTL values" do
    HTTPRR.new(@rr.merge({'ttl'=>'NOPE'})).valid?.should == false
    HTTPRR.new(@rr.merge({'ttl'=>'12C'})).valid?.should == false
    HTTPRR.new(@rr.merge({'ttl'=>'-1'})).valid?.should == false
    HTTPRR.new(@rr.merge({'ttl'=>'0'})).valid?.should == true
    HTTPRR.new(@rr.merge({'ttl'=>'1'})).valid?.should == true
    HTTPRR.new(@rr.merge({'ttl'=>'1337'})).valid?.should == true
    HTTPRR.new(@rr.merge({'ttl'=>'2147483647'})).valid?.should == true
    HTTPRR.new(@rr.merge({'ttl'=>'2147483648'})).valid?.should == false
    HTTPRR.new(@rr.merge({'ttl'=>'99999999999'})).valid?.should == false
  end

  it "correctly validates TYPE values" do
    [nil,true,false,'',1234,'FAKE','AA','AAA','ANAME'].each do |type|
      HTTPRR.new(@rr.merge({'type'=>type})).valid?.should == false
    end
  end

  it "correctly validates RDATA values" do
    [nil,true,false,1,2,3,4].each do |type|
      HTTPRR.new(@rr.merge({'type'=>'TXT','rdata'=>type})).valid?.should == false
    end
  end

  it "doesn't allow RDATA values larger than 2^16" do
    HTTPRR.new(@rr.merge({'type'=>'TXT','rdata'=>'x'*(2**16 - 1)})).valid?.should == true
    HTTPRR.new(@rr.merge({'type'=>'TXT','rdata'=>'x'*(2**16)})).valid?.should == false
    HTTPRR.new(@rr.merge({'type'=>'TXT','rdata'=>'x'*(2**17)})).valid?.should == false
  end
  ...
  it "correctly validates CNAME, NS, PTR records" do
    ['CNAME','NS','PTR'].each do |type|
      ['example.com','example.com.','mx.example.com','a.b.c.e.f.g.h.i.j.example.com',
        ...
        HTTPRR.new(@rr.merge({'type'=>type,'rdata'=>domain})).valid?.should == true
      end
      
      ['10.0.0.1','1000','com','net','org'
      ].each do |domain|
        HTTPRR.new(@rr.merge({'type'=>type,'rdata'=>domain})).valid?.should == false
      end
    end
  end

  it "correctly validates MX records" do
    HTTPRR.new(@rr.merge({'type'=>'MX','rdata'=>'10 mx.example.com.'})).valid?.should == true
    HTTPRR.new(@rr.merge({'type'=>'MX','rdata'=>'10mx.example.com.'})).valid?.should == false
  end
  ...
  it "correctly validates A records" do
    ['0.0.0.0','1.2.3.4','192.168.0.0','10.0.0.0',
     ...
     '10.255.255.255','255.255.255.10','255.255.255.255',
      ...
      HTTPRR.new(@rr.merge({'type'=>'A','rdata'=>ip})).valid?.should == true
    end
    ...
    ['10.10.10.256','10.10.256.10','10.256.10.10','256.10.10.10',
     ...
     '999.999.999.999','9999999.9999999.9999999.9999999',
      ...
      HTTPRR.new(@rr.merge({'type'=>'A','rdata'=>ip})).valid?.should == false
    end
  end

  it "correctly converts to a string" do
    HTTPRR.new(@rr).to_s.should == 'example.com. 0 A 0.0.0.0'
    HTTPRR.new(@rr.merge({'ttl'=>'10','type'=>'MX','rdata'=>'10 mx.example.com'})).to_s.should == 'example.com. 10 MX 10 mx.example.com'
